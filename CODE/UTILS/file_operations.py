...
def move_element(element_path : str, destination : str, filters=()):
    for func in filters:
...
def convert_libraries_in_unpacked(unpacked_path : str, mode : str):
    """
mode   : description

to_xml : Convert to folder with xml files inside and create a xml file listing such files
...
def move_old_entity_library(unpacked_path : str, move_back:bool = False):
    ...
    for directory in ENTITY_LIBRARIES_DIRS:
        ...
        if not move_back:
            move_elements(entity_library_dir, temp_library_dir, (lambda x : not x.endswith(".fcb"),))
        else:
            move_elements(temp_library_dir, entity_library_dir, (lambda x : not x.endswith("blank"),))
